  ...
  _handleMouseDown(e) {
    ...
    var elSize = getOuterSize(React.findDOMNode(this).querySelector('.react-as-dnd-content'));
		...
		if(!limit)
			...
			this._limitOffset = {
				x: [limit.x[0] - this.state.x, limit.x[1] - this.state.x - elSize.width],
				y: [limit.y[0] - this.state.y, limit.y[1] - this.state.y - elSize.height]
			};
  ...
  render() {
    return (
      <div className="react-as-dnd">
        ...
        {
					this.state.dragging && <div className="react-as-dnd-shadow" style={{
            position: 'absolute',
            opacity: 0.5,
